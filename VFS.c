...
void man(char *name)
{
	...
	{
		printf("Description : Used to display information of filr\n");
	...
	{
		printf("Description : Used to oprn existing file\n");
...
int ReadFile(int fd, char*arr,int isize)
{
	...
	if(UFDTArr[fd].ptrfiletable->mode != READ && UFDTArr[fd].ptrfiletable->mode != READ+WRITE)
		return -2;
	if(UFDTArr[fd].ptrfiletable->ptrinode->permission != READ && UFDTArr[fd].ptrfiletable->ptrinode
		->permission != READ+WRITE) return -2;
...
int WriteFile(int fd, char *arr, int isize)
{
	if(((UFDTArr[fd].ptrfiletable->mode) != WRITE) && ((UFDTArr[fd].ptrfiletable->mode) !
		...
		(UFDTArr[fd].ptrfiletable->ptrinode->FileActualSize) = (UFDTArr[fd].ptrfiletable->ptrinode-
...
int LseekFile(int fd,int size,int from)
{
	...
	if((UFDTArr[fd].ptrfiletable->modd == READ) || (UFDTArr[fd].ptrfiletable->mode ==
		READ + WRITE))
	{
		if(from == CURRENT)
		{
